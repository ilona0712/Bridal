...
export default function IsabellaPage() {
  ...
  const dressFromGallery = (location.state as { dress?: DressData } | null)?.dress;
  ...
  useEffect(() => {
    ...
    setMessages([{ type: "bot", text: intro, options: firstQ.options, questionId: firstQ.id }]);
  ...
  const prompt = isComplete ? buildChatGPTPrompt(fullAnswers, dressFromGallery) : "";
  ...
  const handleRequestStyle = async () => {
    ...
    try {
      ...
      const messageContent = `✨ **Custom Dress Request**\n\nHi! I've designed my dream dress using Isabella. Here is the full design prompt — feel free to paste it into ChatGPT / DALL·E to generate a visual, then let's chat about it!\n\n---\n\n${prompt}`;
      ...
      <div className="lg:sticky lg:top-24 space-y-4">
        <div className="bg-white/60 dark:bg-stone-800/60 backdrop-blur-sm rounded-3xl shadow-xl border border-stone-200/50 dark:border-stone-700/50 overflow-hidden">
          <div className="bg-gradient-to-r from-stone-200 via-pink-100/30 to-stone-200 dark:from-stone-700 dark:via-pink-900/20 dark:to-stone-700 px-5 py-4 border-b border-stone-200/50 dark:border-stone-700/50">
            <h3 className="font-serif text-lg text-stone-800 dark:text-stone-100">Base Dress</h3>
            <p className="text-xs text-stone-500 dark:text-stone-400">{dressFromGallery.collection}</p>
          </div>
          <div className="p-5 space-y-4">
            <div className="relative aspect-[3/4] rounded-2xl overflow-hidden">
              ...
              <div className="absolute top-3 left-3 px-3 py-1 bg-stone-800/70 backdrop-blur-sm rounded-full">
                <span className="text-xs text-white">{dressFromGallery.collection}</span>
              </div>
            </div>
            <div className="space-y-2">
              {[
                ...
                ["Sizes", `${dressFromGallery.sizes[0]}–${dressFromGallery.sizes[dressFromGallery.sizes.length - 1]}`],
              ].map(([label, value]) => (
                <p key={label} className="text-sm text-stone-700 dark:text-stone-200">
                  <span className="text-stone-500 dark:text-stone-400">{label}:</span> {value}
                </p>
              ))}
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-stone-200/50 dark:border-stone-700/50">
              ...
              <Link to="/gallery" className="text-xs text-stone-600 hover:text-stone-800 dark:text-stone-400 dark:hover:text-stone-200 underline underline-offset-4">
                Change dress
              </Link>
            </div>
          </div>
            ...
            <div className="bg-white/60 dark:bg-stone-800/60 backdrop-blur-sm rounded-3xl shadow-2xl border border-stone-200/50 dark:border-stone-700/50 overflow-hidden flex flex-col">
              ...
              <div className="bg-gradient-to-r from-stone-200 via-pink-100/30 to-stone-200 dark:from-stone-700 dark:via-pink-900/20 dark:to-stone-700 px-6 py-5 border-b border-stone-200/50 dark:border-stone-700/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="font-serif text-2xl text-stone-800 dark:text-stone-100">Isabella</h2>
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-pink-400 rounded-full animate-pulse" />
                      <span className="text-sm text-stone-600 dark:text-stone-300">Your bridal consultant</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div key={index} className={`flex gap-3 ${message.type === "user" ? "justify-end" : ""}`}>
                      ...
                      {message.type === "bot" && message.options && message.questionId && (
                        <div className="flex flex-wrap gap-2 ml-0">
                          {message.options.map((opt) => {
                            ...
                            const isSelected =
                              isAnswered &&
                              answers[QUESTIONS.find((q) => q.id === message.questionId)?.key as keyof DressAnswers] === opt.value;

                            return (
                              <button
                                key={opt.value}
                                type="button"
                                onClick={() => handleOptionSelect(message.questionId!, opt)}
                                disabled={isAnswered}
                              ...
                              >
                                {isSelected && <Check className="w-3.5 h-3.5 text-pink-500 flex-shrink-0" />}
                                {opt.label}
                              </button>
                            );
                          })}
                    ...
                    {message.type === "user" && (
                      <div className="w-10 h-10 rounded-full flex-shrink-0 overflow-hidden bg-stone-200 dark:bg-stone-600">
                        {myAvatarUrl ? (
                          <img src={myAvatarUrl} alt="You" className="w-full h-full object-cover" />
                        ) : (
                ...
                <div className="border-t border-stone-200/50 dark:border-stone-700/50 p-6 bg-white/40 dark:bg-stone-800/40 space-y-4">
                  ...
                  {!requestSent ? (
                    ...
                    >
                      ...
                      {isSending ? "Sending to designer…" : "Request This Style from Our Designer"}
                  ...
                  {!session?.user && (
                    <p className="text-center text-xs text-stone-500 dark:text-stone-400">
                      <Link to="/login" className="underline underline-offset-2 hover:text-stone-800 dark:hover:text-stone-200">
                        Sign in
                      </Link>{" "}
                  ...
                  <p className="text-center text-xs text-stone-400 dark:text-stone-500">
                    The designer will receive your specifications, generate a visual of your dress, and reach out to discuss details.
                  </p>
                </div>
              )}
