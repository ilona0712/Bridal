...
export default function OwnerChatMessageBubble({
  ...
  return (
    <div
      className={`flex gap-2 sm:gap-3 items-end ${isMine ? "justify-end" : ""}`}
    >
      {!isMine && (
        <button
          type="button"
      ...
      >
        {/* ✅ NAME DISPLAY */}
        <span className="text-[10px] sm:text-xs text-stone-500 px-1">
          {senderLabel}
        </span>
        ...
        <div
          className={[
            ...
            hasAudio
              ? "min-w-[140px] sm:min-w-[200px] max-w-full"
              : "max-w-full",
            isMine
