...
export default function Header({ subtitle, fixed = false }: HeaderProps) {
  ...
  return (
    <header className={`border-b border-stone-200/50 dark:border-stone-700/50 bg-white/80 dark:bg-stone-900/80 backdrop-blur-sm ${fixed ? "fixed" : "sticky"} top-0 z-40 w-full`}>
      <div className="container mx-auto px-4 md:px-6 py-3 md:py-4">
        <div className="flex items-center justify-between w-full relative">
          ...
          <div className="flex md:hidden items-center gap-3">
            ...
            <Link to="/" className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="truncate font-serif text-base text-stone-800 dark:text-stone-100">{settings.logo_text}</h1>
              </div>
            </Link>
          </div>
          ...
          <Link to="/" className="hidden md:flex items-center gap-3 min-w-0 flex-shrink-0">
            ...
            <div className="min-w-0">
              <h1 className="truncate font-serif text-xl text-stone-800 dark:text-stone-100">{settings.logo_text}</h1>
              {tagline && <p className="truncate text-xs text-stone-500 dark:text-stone-400">{tagline}</p>}
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-6 absolute left-1/2 -translate-x-1/2">
            {isAdmin && (
              <Link to="/admin" className="text-sm text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-100 transition-colors">Admin</Link>
            )}
            <Link to="/" className="text-sm text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-100 transition-colors">Home</Link>
            <Link to="/gallery" className="text-sm text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-100 transition-colors">Gallery</Link>
            <Link to="/isabella" className="text-sm text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-100 transition-colors">Consultant</Link>
            {isAdmin ? (
        ...
        {menuOpen && (
          <div className="md:hidden mt-3 pb-3 border-t border-stone-200/50 dark:border-stone-700/50 pt-3 flex flex-col gap-3">
            ...
            {isAdmin && (
              <Link to="/admin" onClick={() => setMenuOpen(false)} className="text-sm text-stone-600 dark:text-stone-300 hover:text-stone-800 dark:hover:text-stone-100 px-1">Admin</Link>
            )}
            <Link to="/" onClick={() => setMenuOpen(false)} className="text-sm text-stone-600 dark:text-stone-300 hover:text-stone-800 dark:hover:text-stone-100 px-1">Home</Link>
            <Link to="/gallery" onClick={() => setMenuOpen(false)} className="text-sm text-stone-600 dark:text-stone-300 hover:text-stone-800 dark:hover:text-stone-100 px-1">Gallery</Link>
            <Link to="/isabella" onClick={() => setMenuOpen(false)} className="text-sm text-stone-600 dark:text-stone-300 hover:text-stone-800 dark:hover:text-stone-100 px-1">Consultant</Link>
            {!session && (
              <Link to="/login" onClick={() => setMenuOpen(false)} className="text-sm text-stone-600 dark:text-stone-300 hover:text-stone-800 dark:hover:text-stone-100 px-1">Sign In</Link>
            )}
            {session && (
              <button onClick={handleSignOut}
